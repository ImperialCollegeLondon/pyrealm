...
# PUT IN TYPING: : list[tuple[Callable, tuple[_ArrayTesting, ...]]]?
...
def create_full_args(call_info, default=np.array((15, 10, 5))):
    """Generates required array arguments at full size."""

    full_default = default if call_info.shape is None else call_info.shape
    full_args = dict()
    for arg in call_info.array_args:
        full = full_default if arg.shape is None else arg.shape
        full_args[arg.name] = np.full(full, arg.fill_value)
...
def equality_testing(full: Any, alt: Any, call_info: _ArrayTesting):
    ...
    tested, but classes are hard to test so a set of defined array attributes are tested
    instead.
    """
...
def test_array_broadcasting(to_broadcast_test, collapsed_axis_combinations):
    ...
    full_args = create_full_args(call_info)
    ...
    for axis_combination in collapsed_axis_combinations:
        ...
        reduced_args_template = dict()

        # Generate the reduced axis shape for each attribute combination.
        # TODO - need mechanism to handle inputs with different dims, like time?
        for arg in call_info.array_args:
            reduced = np.array(full_args[arg.name].shape)
            reduced[axis_combination] = 1
            reduced_args_template[arg.name] = np.full(reduced, arg.fill_value)

        # Restore one array attribute at a time to full shape and check the broadcast
        # result works.
        for arg in call_info.array_args:
            reduced_args = reduced_args_template.copy()
            reduced_args[arg.name] = full_args[arg.name]
