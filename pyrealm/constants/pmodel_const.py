...
class PModelConst(ConstantsClass):
    ...
    * **Dark respiration**. Value taken from :cite:t:`Atkin:2015hk` for C3 herbaceous
    ...
    arrhenius_mode: str = "M2002"
    """The calculation mode to use with the modified Arrhenius equation in
    :func:`~pyrealm.pmodel.functions.calc_kattge_knorr_arrhenius_factor`. This mode
    should be consistently within an analysis, so forms part of the model constants. The
    two options are ``M2002`` and ``J1942``, see :cite:t:`murphy:2021a` for details."""
