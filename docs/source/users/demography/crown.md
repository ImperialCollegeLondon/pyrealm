...
q_m = calculate_canopy_q_m(m=m, n=n[:, None])
z_max_prop = calculate_canopy_z_max_proportion(m=m, n=n[:, None])
```
...
allometry = flora.get_allometries(stem_height=stem_height)
```
...
crown_profiles = flora.get_canopy_profile(
    z=z,
    crown_area=allometry["crown_area"],
    stem_height=allometry["stem_height"],
    r0=allometry["canopy_r0"],
    z_max=allometry["canopy_z_max"],
)
```
...
for pft_idx, offset, colour in zip((0, 1, 2), (0, 5, 12), ("r", "g", "b")):
    ...
    ax.plot(
        ...
        [allometry["canopy_z_max"][pft_idx]] * 2,
        color=colour,
        linewidth=1,
        linestyle=":",
    )
...
allometry2 = flora2.get_allometries(stem_height=stem_height)


# Calculate the crown profile across those heights for each PFT
crown_profiles2 = flora2.get_canopy_profile(
    z=z,
    crown_area=allometry2["crown_area"],
    stem_height=allometry2["stem_height"],
    r0=allometry2["canopy_r0"],
    z_max=allometry2["canopy_z_max"],
)
```
