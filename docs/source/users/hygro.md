...
The {class}`~pyrealm.pmodel.PModelEnvironment.PModelEnvironment` class requires vapour pressure
...
* Rescale the output of {func}`~pyrealm.hygro.convert_vp_to_vpd` from
  kPa to Pa, before using it in {class}`~pyrealm.pmodel.PModelEnvironment.PModelEnvironment`.
